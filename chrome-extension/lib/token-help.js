/** Token acquisition guides for new users. */
...
export const TOKEN_HELP = {
  github: {
    ...
    note: 'Token 仅保存在你的浏览器本机，不会同步到云端。',
  },
  gitlab: {
    ...
    steps: [
      ...
      '复制以 <code>glpat-</code> 开头的字符串并粘贴到设置中',
    ],
...
/**
 * Wire up ? buttons and optional overlay elements.
 * @param {object} ids - DOM ids for overlay pieces (all optional if overlay omitted)
 */
export function initTokenHelp(ids = {}) {
  ...
  function openHelp(platform) {
    ...
    // Fallback: alert-style for pages without overlay markup
